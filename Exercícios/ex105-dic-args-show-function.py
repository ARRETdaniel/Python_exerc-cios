...
def notas_aluno(*args, situacao=False):
    ...
    info_alunos = dict(total=len(args), maior=max(
        args), menor=min(args), media=sum(args) / len(args))
    print(info_alunos)
    if situacao:
...
resp = notas_aluno(5.5, 2, 2, 6.5, situacao=True)
print(f'Dicionário de dados:{cor["amarelo"]} {resp} {cor["limpa"]} ')
print()
help(notas_aluno)
